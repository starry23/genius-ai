 ...
 * @LastEditTime: 2023-07-01 11:27:21
-->
<template>
  <div class="payment_order_list-warp">
    <div class="payment_order_list-head">
      <el-button
        @click="deleteAllInvalidOrderConfirm"
...
<script>
import { managerOrders, deleteUselessOrders } from "@/api/paymentOrderList";
...
export default {
  ...
  methods: {
    // 删除无效订单确认
    deleteAllInvalidOrderConfirm() {
      this.$confirm("此操作将永久删除无效订单, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
        this.deleteAllInvalidOrderFn();
      });
    },
    // 删除无效订单
    async deleteAllInvalidOrderFn() {
      try {
        let res = await deleteUselessOrders();
        if (res.code === 200) {
          this.$message({
            type: "success",
            message: "删除成功!",
